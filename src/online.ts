...
export function compareMetadata(
  ...
  if (metadata.name !== expectedName) {
    messages.push(
      `name: expected ${expectedName}, received ${String(metadata.name)}`,
    );
  }
  ...
  if (entry.version !== undefined && metadata.version !== entry.version) {
    messages.push(
      `version: expected ${entry.version}, received ${String(metadata.version)}`,
    );
  }

  const npmPackage = metadata.packages?.find(
    (item) => item.registryType === "npm",
  );
  if (entry.distribution?.npm !== undefined) {
    if (npmPackage?.identifier !== entry.distribution.npm.package) {
      messages.push(
        "npm package identifier differs from the curated published package",
      );
    }
    if (npmPackage?.version !== entry.distribution.npm.version) {
      messages.push(
        "npm package version differs from the curated published package",
      );
    }
  }
  ...
  for (const item of metadata.packages ?? []) {
    if (item.transport?.type !== undefined)
      sourceTransports.add(item.transport.type);
  }
  ...
  for (const transport of sourceTransports) {
    if (!entry.interfaces.transports.includes(transport)) {
      messages.push(
        `transport ${transport} is declared upstream but absent from the entry`,
      );
    }
  }
  return messages;
...
async function fetchJson(url: string): Promise<unknown> {
  const response = await fetch(url, {
    headers: {
      ...
      "user-agent": "agent-tool-server-registry",
    },
...
export async function verifyEntryOnline(
  entry: ServerEntry,
  ...
  try {
    ...
    return {
      ...
      messages: [
        missing
          ? `committed server.json is missing at ${metadataUrl}`
          : message,
      ],
    };
  }
  ...
  if (
    ...
    !metadata.$schema.startsWith("https://")
  ) {
    messages.push("server.json must reference an HTTPS official MCP schema");
  } else {
    try {
      const schema = (await fetchJson(metadata.$schema)) as AnySchema;
